...
function extractFirstImage(content: string): string | undefined {
  // frontmatterのimageフィールドをチェック（まだ処理していないので、本文から抽出）
  ...
  if (markdownMatch && markdownMatch[1]) {
    // タイトル部分（引用符で囲まれた部分）を除外
    const imagePath = markdownMatch[1].trim()
    // スペースで分割して最初の部分（画像パス）だけを取得
 ...
 */
function getAllMarkdownFiles(dir: string, baseDir: string, fileList: string[] = []): string[] {
